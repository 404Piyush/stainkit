...
ARGS=(
  ...
  --num-streams "${STREAMS}"
  --benchmark
  --csv     "${BENCH_DIR}/last_run.csv"
)
if [[ ${CPU} -eq 1 ]]; then ARGS+=(--cpu); fi
if [[ ${PINNED} -eq 1 ]]; then ARGS+=(--pinned); else ARGS+=(--no-pinned); fi
...
echo "stainkit run.sh: benchmark in ${BENCH_DIR}/last_run.csv"
