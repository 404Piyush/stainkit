...
stk::Image MakeSyntheticHePatch(int w, int h) {
  auto img =
      stk::MakeImage(static_cast<std::size_t>(w), static_cast<std::size_t>(h),
                     stk::PixelLayout::kRgb);
  for (int y = 0; y < h; ++y) {
    for (int x = 0; x < w; ++x) {
      ...
      img.pixels[y * img.stride + 3 * x + 0] = static_cast<std::uint8_t>(
          std::clamp(220.0f - 90.0f * core, 0.0f, 255.0f));
      img.pixels[y * img.stride + 3 * x + 1] = static_cast<std::uint8_t>(
          std::clamp(180.0f - 80.0f * core, 0.0f, 255.0f));
      img.pixels[y * img.stride + 3 * x + 2] = static_cast<std::uint8_t>(
          std::clamp(190.0f - 30.0f * core, 0.0f, 255.0f));
    }
  }
  return img;
}
...
TEST(Pipeline, CpuBaselineRun) {
  ...
  double ms = 0.0;
  auto out =
      stk::CpuReferenceStainNormalise(img, stk::PipelineParams{}, target, &ms);
