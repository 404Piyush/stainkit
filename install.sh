...
# benchmark binary and (optionally) the Python bindings. Tested on:
...
#   ./install.sh --python                 # build the pybind11 module
#   ./install.sh --cuda-arch=75,86        # custom CUDA architectures
#   ./install.sh -v / --verbose           # trace every command
# ---------------------------------------------------------------------------

# `set -e` exits on error; we deliberately avoid `-u` because it can
# abort on empty array expansion under certain bash versions.
set -eo pipefail
...
print_args() {
    echo "stainkit install.sh: BUILD_TYPE=${BUILD_TYPE} BUILD_PYTHON=${BUILD_PYTHON} BUILD_TESTS=${BUILD_TESTS} CUDA_ARCHS=${CUDA_ARCHS}" >&2
}
...
if ! command -v nvcc >/dev/null 2>&1; then
    ...
    if [[ -x /usr/local/cuda/bin/nvcc ]]; then
        ...
        echo "stainkit: using nvcc at /usr/local/cuda/bin/nvcc" >&2
    else
...
rm -rf CMakeCache.txt CMakeFiles CMakeUserPresets.json

# Build the cmake invocation as an array so empty EXTRA_CMAKE_ARGS does
# not crash under any bash version.
CMAKE_INVOCATION=(
    cmake
...
if [[ "${#EXTRA_CMAKE_ARGS[@]}" -gt 0 ]]; then
    CMAKE_INVOCATION+=("${EXTRA_CMAKE_ARGS[@]}")
fi
CMAKE_INVOCATION+=("..")
