...
__global__ void RgbToOdKernel(const float* __restrict__ in, float* __restrict__ out,
                              std::size_t npix) {
...
__global__ void OdToRgbKernel(const float* __restrict__ in, float* __restrict__ out,
                              std::size_t npix) {
  const std::size_t idx = static_cast<std::size_t>(blockIdx.x) * blockDim.x +
                          threadIdx.x;
  if (idx >= npix) return;
