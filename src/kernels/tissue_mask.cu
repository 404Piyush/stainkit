...
__global__ void HistogramKernel(const float* __restrict__ d_lum, int* __restrict__ d_hist,
                                std::size_t npix) {
...
__global__ void ThresholdKernel(const float* __restrict__ d_lum,
  ...
  if (threadIdx.x == 0) {
    ...
    for (int t = 0; t < kBins; ++t) {
      ...
      if (weight_back <= 0.0f) continue;
      const float weight_fore = total - weight_back;
      if (weight_fore <= 0.0f) break;
...
__global__ void BinariseKernel(const float* __restrict__ d_lum, float threshold,
                               std::uint8_t* __restrict__ d_out, std::size_t npix) {
  const std::size_t idx = static_cast<std::size_t>(blockIdx.x) * blockDim.x +
                          threadIdx.x;
  if (idx >= npix) return;
...
void RgbToLuminance(const float* d_in_rgb, std::size_t width, std::size_t height,
                    float* d_out_lum, void* stream) {
  if (d_in_rgb == nullptr || d_out_lum == nullptr) return;
  const std::size_t npix  = width * height;
  const int         block = 256;
  const int         grid  = static_cast<int>((npix + block - 1) / block);
  cudaStream_t      s     = AsStream(stream);
  LumaKernel<<<grid, block, 0, s>>>(d_in_rgb, d_out_lum, npix);
}
...
float OtsuThresholdHost(const float* d_in_lum, std::size_t width,
  ...
  for (int t = 0; t < kBins; ++t) {
    ...
    if (weight_b <= 0.0f) continue;
    const float weight_f = total - weight_b;
    if (weight_f <= 0.0f) break;
...
float OtsuThresholdDevice(const float* d_in_lum, std::size_t width,
  ...
  cudaMemsetAsync(d_hist, 0, kBins * sizeof(int), s);

  HistogramKernel<<<grid, block, kBins * sizeof(unsigned int), s>>>(
      d_in_lum, d_hist, npix);
  ThresholdKernel<<<1, block, kBins * sizeof(unsigned int), s>>>(
...
void ThresholdToMask(const float* d_in_lum, std::size_t width, std::size_t height,
                     float threshold, std::size_t radius,
                     std::uint8_t* d_out_mask, void* stream) {
  if (d_in_lum == nullptr || d_out_mask == nullptr) return;
  const std::size_t npix  = width * height;
  const int         block = 256;
  const int         grid  = static_cast<int>((npix + block - 1) / block);
  cudaStream_t      s     = AsStream(stream);
  BinariseKernel<<<grid, block, 0, s>>>(d_in_lum, threshold, d_out_mask, npix);
  if (radius > 0) {
