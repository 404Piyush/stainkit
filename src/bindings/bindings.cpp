...
stainkit::Image FromNumpy(py::array_t<std::uint8_t, py::array::c_style | py::array::c_contiguous> arr) {
...
PYBIND11_MODULE(gpustain, m) {
  ...
  m.def("read_image", [](const std::string& path) {
    return FromNumpy(stainkit::ReadImage(fs::path{path}, 3));
  }, "Read an RGB image from disk into a (H, W, 3) uint8 array.");

  m.def("write_image", [](py::array_t<std::uint8_t> arr, const std::string& path) {
  ...
  py::class_<stainkit::PipelineResult>(m, "Result")
      ...
      .def("normalised_array", [](const stainkit::PipelineResult& r) {
  ...
  m.def("run", [](py::array_t<std::uint8_t> arr, const std::string& target_name) {
    auto img    = FromNumpy(arr);
    auto pl     = stainkit::Pipeline::Make();
    if (!pl) throw std::runtime_error("CUDA not available");
    stainkit::StainTarget target;
    target.name = target_name;
    if (target_name == "he-royal") {
      target.matrix.values = {0.5826f, 0.7660f, 0.2723f, 0.2053f, 0.8520f, 0.4830f};
    } else if (target_name == "he-icm") {
      target.matrix.values = {0.6500f, 0.7040f, 0.2860f, 0.0700f, 0.9900f, 0.1200f};
    }
  ...
  py::class_<stainkit::BenchmarkRecord>(m, "BenchmarkRecord")
      ...
      .def_readonly("cpu_baseline_ms", &stainkit::BenchmarkRecord::cpu_baseline_ms);
}
